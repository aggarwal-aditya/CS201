...
int main() {
    ...
    //Start of Multiplication Operation; Comment Out for Doing Addition
    term *head_product_poly;
    head_product_poly = prodTwoPolynomial(head_poly_1, head_poly_2, &res);
    sort(head_product_poly);
    merge(head_product_poly);
    printf("\nThe product of two polynomials is\n");
    while (head_product_poly->next != NULL) {
        if (head_product_poly->coeff != 0) {
            printf("%dx^%d ", head_product_poly->coeff, head_product_poly->power);
            if (head_product_poly->next->next != NULL && head_product_poly->next->coeff >= 0)
                printf(" + ");
        }
        head_product_poly = head_product_poly->next;
    }
